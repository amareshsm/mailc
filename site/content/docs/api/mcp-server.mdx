...
| `data` | `record` | no | Template data for \{\{variable\}\} interpolation, mc-if conditions, and mc-for-each loops. Optional. |
...
| Parameter | Type | Required | Description |
|---|---|---|---|
| `source` | `string` | yes | mailc markup string. Expressions (\{\{var\}\}), mc-if conditions, and mc-for-each loops are walked statically — no data is evaluated. |
